...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        ocl_fillBufferWithZeros.exec(gpu::WorkSize(GROUP_SIZE, (n + GROUP_SIZE - 1) / GROUP_SIZE), buffer1_gpu, n);
        ocl_fillBufferWithZeros.exec(gpu::WorkSize(GROUP_SIZE, (n + GROUP_SIZE - 1) / GROUP_SIZE), buffer2_gpu, n);
        ocl_fillBufferWithZeros.exec(gpu::WorkSize(GROUP_SIZE, (n + GROUP_SIZE - 1) / GROUP_SIZE), buffer3_gpu, n);
        ocl_fillBufferWithZeros.exec(gpu::WorkSize(GROUP_SIZE, (n + GROUP_SIZE - 1) / GROUP_SIZE), buffer4_gpu, n);
        
        ocl_radixSort01LocalCounting.exec(gpu::WorkSize(1, 1), input_gpu, buffer1_gpu, 0, n);
        ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(1, 1), buffer1_gpu, buffer2_gpu, n);
        ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(1, 1), buffer2_gpu, buffer3_gpu, 0, n);
        ocl_radixSort04Scatter.exec(gpu::WorkSize(1, 1), input_gpu, buffer3_gpu, buffer_output_gpu, 0, n);
